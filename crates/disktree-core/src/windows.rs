...
/// Whether `path` is `base` or inside it, compared the way Windows compares
/// names: without regard to case, so `C:\WINDOWS\System32` is inside
/// `C:\Windows`, and with `\\?\C:\` the same place as `C:\`.
pub fn within(path: &Path, base: &Path) -> bool {
    folded(path).starts_with(&folded(base))
}

/// Whether `left` and `right` name the same place, as [`within`] compares.
pub fn same(left: &Path, right: &Path) -> bool {
    folded(left) == folded(right)
}

/// Each component in the form Windows would treat as equal.
